...
{
	int usetime;
	usetime=(etime[0]-'0')*10*60+(etime[1]-'0')*60+10*(etime[2]-'0')+(etime[3]-'0')-(btime[0]-'0')*10*60-(btime[1]-'0')*60-10*(btime[2]-'0')-(btime[3]-'0');
    return usetime;
}
	...
	if(data!=NULL)
	{
		...
		while(1)
		{
			...
			if(ch==13)
			{
				...
				way_fp=file_open(filename);
				file_readall(way_fp,way_head,IWAY);
	            way_printpage_nosearch(way_head);
				fclose(way_fp);
			}
			if(ch=27)
			{
